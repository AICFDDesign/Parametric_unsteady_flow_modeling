...
def train_epoch(model, data, optimizer, beta, device):
    ...
    for batch in data:
        ## # batch is a tensor with shape: (batch_size, 2, 88, 300)
        ...
        para = para.float()
        #print(f"w: {w.shape}, para: {para.shape}")
        bt = w.shape[0]
        w = w.reshape(bt, -1, 200, 200).float()  # Reshape w to (bt, 1, 200, 200)

        # 调用模型的 forward 方法
        rec, mean, logvariance = model((w, para))  # 传入元组 (w, para)
        loss, MSE, KLD = loss_function(rec, w, mean, logvariance, beta)
...
def test_epoch(model, data, beta, device):
    ...
    with torch.no_grad():
        ...
        for batch in data:
            ...
            rec, mean, logvariance = model((w, para))
            loss, MSE, KLD = loss_function(rec, w, mean, logvariance, beta)
