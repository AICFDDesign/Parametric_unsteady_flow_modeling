...
def get_pvae(latent_dim,input_paradim): 
    """
    A function to obtain the para_beta-VAE model

    Args: 
        latent_dim : (int) The latent-variable dimension
        input_paradim : (int) The input parameters dimension
    
    Returns:
        model      : (torch.nn.Module)The para_beta-VAE model
    """

    from nns.para_beta_vae import parasVAE

    return parasVAE(latent_dim,input_paradim)
