...
def spatial_Mode(   fname,
    ...
    Args:
        ...
        if_modes        : (bool) IF generate modes
    ...
    if if_modes:
        _, para_train   = train_data.tensors
        _, para_gen     = gen_data.tensors
        para_train = para_train.reshape(-1, n_train, para_dim)
        para_gen   = para_gen.reshape(-1, n_train + n_test, para_dim)
        zero_output, modes, modes_para_train, modes_para_gen = get_spatial_modes(model, latent_dim, device, para_train, para_gen)
        print('INFO: Spatial mode generated')
    else:
        zero_output, modes, modes_para_train, modes_para_gen = None, None, None, None
    # import scipy.io
    # scipy.io.savemat('mode_para.mat', {
    #     'modes_para_train': modes_para_train,
    #     'modes_para_gen':modes_para_gen,
    # })
    ...
    if if_nlmode:
        NLvalues, NLmodes = getNLmodes(model, order_all, latent_dim, device)
    ...
    is_save = createModesFile(fname, fname_predw, model, latent_dim, 
                            ...
                            NLvalues, NLmodes,
                            Ek_tr, Ek_te, Ek_gen,
                            zero_output, modes, modes_para_train, modes_para_gen)
...
def sindy_rec_analysis(   fname,
    ...
    import scipy.io
    print(f"INFO: START sindy analysis")
    sindy_latent = scipy.io.loadmat(fname)['v_train_sindy_predicted']
    print(f"successfully load sindy latent data from {fname}")
    cases = sindy_latent.shape[0]
    Ek_sindy = np.zeros((cases, sindy_latent.shape[1]), dtype=np.float32)
    print(f"INFO: The cases of sindy latent is {cases}")
    w,_ = train_data.tensors
    w = w.reshape(-1, n_train, 200, 200)
    for k in range(cases):
        sindy_latent_rec = decode(model, sindy_latent[k,:,:], device)
        for j in range(n_train):
            Ek_sindy[k,j] = get_Ek(w[k,j,:,:], sindy_latent_rec[j,:,:])
    print(f"INFO: sindy analysis DONE!")
    return Ek_sindy
...
def calc_paramode(model, latent_dim, mode, device, case, para_data):
    """
    ...
    with torch.no_grad():
        mode_para = model.decoder(torch.from_numpy(z_sample).to(device)*para_coder).cpu().numpy()[0, :, :]
        
    return mode_para

#--------------------------------------------------------
def get_spatial_modes(model, latent_dim, device, para_data_train, para_data_gen):
    ...
    case_train = para_data_train.shape[0]
    case_gen = para_data_gen.shape[0]
    print('cases of train:',case_train,',cases of gen:',case_gen)

    modes = np.zeros((latent_dim, zero_output.shape[1], zero_output.shape[2], zero_output.shape[3]))
    modes_para_train = np.zeros((latent_dim, case_train, zero_output.shape[2], zero_output.shape[3]))
    modes_para_gen = np.zeros((latent_dim, case_gen, zero_output.shape[2], zero_output.shape[3]))

    for mode in range(latent_dim):
        modes[mode, :, :, :] = calcmode(model, latent_dim, mode, device)
        for i in range(case_train):
            modes_para_train[mode, i, :, :] = calc_paramode(model, latent_dim, mode, device, i, para_data_train)
        for j in range(case_gen):
            modes_para_gen[mode, j, :, :] = calc_paramode(model, latent_dim, mode, device, j, para_data_gen)

    return zero_output, modes, modes_para_train, modes_para_gen


#--------------------------------------------------------
def getNLmodes(model, order, latent_dim, device):
    """
    ...
    NLvalues = np.arange(-2, 2.5, 0.5)
    print("NLvalues", NLvalues) 
    NLmodes = np.zeros((latent_dim, NLvalues.shape[0],  zero_output.shape[2], zero_output.shape[3]),
                        dtype=np.float32)
    print("NLmodes shape:", NLmodes.shape)
    
    for j,i in enumerate(order):
        for idx, value in enumerate(NLvalues):
            latent = np.zeros((1, latent_dim), dtype=np.float32)
            latent[0, i] = value
            NLmodes[j,idx,:,:] = decode(model, latent, device)
...
#--------------------------------------------------------
def get_Ek_t_sindy(model, data, n_data, device):
    """
...
def createModesFile(fname,
    ...
    #         # mu
    #         f.create_dataset('vector_train', data=means_train)
    #         f.create_dataset('vector_test', data=means_test)
    #         f.create_dataset('vector_gen', data=means_gen)
            
    #         # Sigma
    #         f.create_dataset('stds_vector_train', data=stds_train)
    #         f.create_dataset('stds_vector_test', data=stds_test)
    #         f.create_dataset('stds_vector_gen', data=stds_gen)
    ...
    # f.close()
    # print(f"INFO: Post-processing results has been saved as dataset: {fname}.hdf5")
    ...
    #             f.create_dataset('vector_test', data=means_test)
    #     f.close()
    #     print(f"Saving file to: {pathsBib.data_path}")
    # except:
    #     print(f"Error: Failed to save latent_data.h5py file")
