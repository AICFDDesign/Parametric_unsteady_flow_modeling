...
#-------------------------------------------------
    ...
    def post_process(self,if_window=True,if_pmap=False):
        """
        ...
        if if_pmap:
            ...
            test_cases = test_data.shape[0]
            gen_cases = gen_data.shape[0]
            print(f"INFO: The test data has {test_cases} cases, the gen data has {gen_cases} cases")
            print(f"INFO: Start to compute the Poincare Map of test data")
            InterSec_test = []
            InterSec_pred = []
            InterSec_gen = []
            InterSec_pred_gen = []
            for i in tqdm(range(test_cases)):
                InterSec_pred.append(Intersection(Preds[i,:,:],     planeNo=planeNo,postive_dir=postive_dir))
                InterSec_test.append(Intersection(test_data[i,:,:], planeNo=planeNo,postive_dir=postive_dir))
            print(f"INFO: Start to compute the Poincare Map of gen data")
            for i in tqdm(range(gen_cases)):
                InterSec_pred_gen.append(Intersection(Preds_gen[i,:,:],     planeNo=planeNo,postive_dir=postive_dir))
                InterSec_gen.append(Intersection(gen_data[i,:,:], planeNo=planeNo,postive_dir=postive_dir))
            print(f"INFO: The Poincare Map has been computed")
