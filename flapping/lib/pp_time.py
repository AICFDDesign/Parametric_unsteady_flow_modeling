...
def make_Prediction(test_data, model,
    ...
    Preds = Preds.reshape(-1, Num_t, laten_dim)
    Cases = Preds.shape[0]
    print(f"The number of cases = {Cases}")
    seq_len = Num_t #max([Preds.shape[1],Preds.shape[2]])
    print(f"The sequence length = {seq_len}")

    for k in range(Cases):
        print(f"------------Case {k}/{Cases}--------------")
        for i in tqdm(range(in_dim,seq_len-next_step)):
            
            feature = Preds[k,i-in_dim:i,:].reshape(-1,in_dim,laten_dim)
...
def Sliding_Window_Error(test_data,
    ...
    def Rell2norm(predictions, targets):
        return np.sqrt( np.sum( (predictions - targets) ** 2, axis=1 ) / np.sum( targets ** 2, axis=1 ) )
    
    model.to(device)
    model.eval()

    SeqLen = test_data.shape[1]
    Cases = test_data.shape[0]
    print(f"The number of All cases = {Cases}")
    err = np.zeros((Cases,window))
    rel_err = np.zeros((Cases,window))
    ...
    for k in range(Cases):
        ...
        for init in tqdm(range(in_dim,SeqLen-window,1)):
            temporalModes_pred = copy.deepcopy(test_data[k,:,:])
            ...
            if error is None:
                error = l2norm(temporalModes_pred[init:init+window,:], test_data[k, init:init+window,:])
                rel_error = Rell2norm(temporalModes_pred[init:init+window,:], test_data[k, init:init+window,:])
                n = 1
            else:
                error = error + l2norm(temporalModes_pred[init:init+window,:], test_data[k, init:init+window,:])
                rel_error = rel_error + Rell2norm(temporalModes_pred[init:init+window,:], test_data[k, init:init+window,:])
                n += 1

        print(n)
        err[k,:] = error / n 
        rel_err[k,:] = rel_error / n
        print(f"Error: {err[k,:]}")
        print(f"Relative Error: {rel_err[k,:]}")
        print(error.shape)
...
def make_physical_prediction_all_case(vae,
                            pred_latent,
                            true_latent,
                            device):
    """
    ...
    import numpy as np
    from lib.pp_space import decode
    latent_dim = true_latent.shape[-1]
    cases = true_latent.shape[0]

    rec_vae     =   decode(vae.model,true_latent.reshape(-1,latent_dim),device)

    rec_pred    =   decode(vae.model,pred_latent.reshape(-1,latent_dim),device)

    return rec_vae.reshape(cases,-1,200,200), rec_pred.reshape(cases,-1,200,200)
...
def PDF(InterSecX,InterSecY,
    ...
    print(f"InterSecX shape: {InterSecX.shape}, InterSecY shape: {InterSecY.shape}")
